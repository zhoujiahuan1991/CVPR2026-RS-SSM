...
class BaseDecodeHead_clips_flow(nn.Module, metaclass=ABCMeta):
    ...
    def losses(self, seg_logit, seg_label):
        ...
        if seg_logit.shape[1]==seg_label.shape[1]+1:     # k+1
        ...
        elif seg_logit.shape[1]==seg_label.shape[1]:
            seg_logit_ori=seg_logit
            batch_size, num_clips, _, h ,w=seg_logit_ori.shape
            seg_logit_ori=seg_logit_ori.reshape(batch_size*num_clips,-1,h,w)
            seg_logit_lastframe=seg_logit[:,-1]
            ...
            print(seg_logit.shape[1], seg_label.shape[1])
            assert (1==0)
...
class BaseDecodeHead_clips_flow_ours(nn.Module, metaclass=ABCMeta):
    """Base class for BaseDecodeHead_clips_flow.
    ...
    def __init__(self,
                 ...
                 *,
                 ...
                 align_corners=False,
                 num_clips=5):
        super(BaseDecodeHead_clips_flow_ours, self).__init__()
        self._init_inputs(in_channels, in_index, input_transform)
        self.channels = channels
        self.num_classes = num_classes
        self.dropout_ratio = dropout_ratio
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg,batch_size, num_clips,img=None):
        ...
        Returns:
            dict[str, Tensor]: a dictionary of loss components
        """
        seg_logits, detail_logits, align_loss = self.forward(inputs,batch_size, num_clips,img)
        losses = self.losses(seg_logits, detail_logits, gt_semantic_seg, align_loss)
        return losses
    ...
    @force_fp32(apply_to=('seg_logit', ))
    def losses(self, seg_logit, detail_logits, seg_label, align_loss):
        """Compute segmentation loss."""
        ...
        if single_flag:
            loss['loss_seg'] = self.loss_decode(
                ...
                seg_label_ori,
                weight=seg_weight,
                ignore_index=self.ignore_index) + 0.01 * self.loss_decode(
                detail_logits,
                seg_label_lastframe,
                weight=seg_weight,
                ignore_index=self.ignore_index) + 0.01 * align_loss
        else:
            loss['loss_seg'] = 0.5 * self.loss_decode(
                seg_logit_ori,
                seg_label_ori,
