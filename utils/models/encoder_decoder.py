...
class EncoderDecoder_clips(BaseSegmentor):
    ...
    def aug_test(self, imgs, img_metas, rescale=True):
        ...
        img = torch.stack(imgs[0], dim=1)
        if img.dim()==5:
            batch_size, num_clips, _, h, w = img.size()
        img = img.reshape(batch_size * num_clips, -1, h, w)
        seg_logit = self.inference(img, img_metas[0], rescale, batch_size, num_clips)
        for i in range(1, len(imgs)):
            img = torch.stack(imgs[i], dim=1)
            if img.dim()==5:
                batch_size, num_clips, _, h, w = img.size()
            img = img.reshape(batch_size * num_clips, -1, h, w)
            cur_seg_logit = self.inference(img, img_metas[i], rescale, batch_size, num_clips)
