...
def multi_gpu_test(model,
    ...
    if mamba_mode:
        ...
        test_loader = build_dataloader(
            subset,
            samples_per_gpu=1,
            workers_per_gpu=data_loader.num_workers, # or cfg.data.workers_per_gpu
            dist=False,
            shuffle=False
        )
    else:
    ...
    if rank == 0:
        prog_bar = mmcv.ProgressBar(len(test_loader.dataset) * world_size if mamba_mode else len(dataset))
    ...
    if gpu_collect:
        print("Collecting results by GPU...")
        results = collect_results_gpu(results, len(dataset))
    else:
        print("Collecting results by CPU...")
