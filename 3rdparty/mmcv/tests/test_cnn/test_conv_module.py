...
def test_conv_module():
    ...
    # conv + norm + act
    conv = ConvModule(3, 8, 2, norm_cfg=dict(type='SyncBN', requires_grad=True))
...
def test_bias():
    ...
    with pytest.warns(UserWarning) as record:
        ConvModule(3, 8, 2, bias=True, norm_cfg=dict(type='SyncBN', requires_grad=True))
    assert len(record) == 1
...
def test_order():
    ...
    # ('norm', 'conv', 'act')
    conv = ConvModule(
        3, 8, 2, norm_cfg=dict(type='SyncBN', requires_grad=True), order=('norm', 'conv', 'act'))
    out = conv('input')
